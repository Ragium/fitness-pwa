...
export class StatsComponent implements OnInit, AfterViewInit, OnDestroy {
      ...
      if (user) {
        ...
        this.workoutService.getByUserId(user.uid).subscribe({
          next: (workouts) => {
            ...
            console.log('Feldolgozott edzések:', this.recentWorkouts);
  ...
  calculateTypeDistribution(workouts: Workout[]): { [key: string]: number } {
      ...
      if (workout.exercises && workout.exercises.length > 0) {
        workout.exercises.forEach(exercise => {
          if (exercise.type) {
            ...
            console.log('Exercise type:', exercise.type, 'Converted to:', type); // Debug log
    ...
    // Távolítsuk el a 0 értékű típusokat
    ...
    console.log('Final type counts:', typeCounts); // Debug log
    return typeCounts;
  }
    ...
    workouts.forEach(workout => {
      ...
      if (workoutDate >= weekStart && workoutDate <= today) {
        ...
        if (dayIndex >= 0 && dayIndex < 7) {
          weeklyWeights[dayIndex] += this.calculateTotalWeight(workout);
        }
      }
    });
