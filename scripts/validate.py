...
AI_DIR = DOCS_DIR / "ai"
INTERVIEWS_DIR = DOCS_DIR / "interviews"
MARKET_DIR = DOCS_DIR / "market"
...
def validate_prd() -> None:
    prd_path = DOCS_DIR / "spec" / "prd.yaml"
    ...
    # Támogassuk a repo-root és a docs-mappához viszonyított hivatkozást is.
    candidates = [REPO_ROOT / ref_value, DOCS_DIR / ref_value]
    if not any(candidate.exists() for candidate in candidates):
        raise ValueError(
            "A first_tech_decision_ref által hivatkozott ADR nem található. "
            "Elvárt útvonal például: docs/adr/0003-pwa-offline-first-architecture.md"
        )
...
def validate_ai_log(min_entries: int) -> None:
    ...
    if not log_path.exists():
        raise ValueError("Hiányzó AI napló: docs/ai/ai_log.jsonl")
...
def validate_interviews() -> None:
    if not INTERVIEWS_DIR.exists():
        raise ValueError("Hiányzó mappa: docs/interviews")
...
def validate_competitors() -> None:
    ...
    if not csv_path.exists():
        raise ValueError("Hiányzó market/competitors.csv fájl a docs/market mappában")
...
def validate_adr() -> None:
    if not ADR_DIR.exists():
        raise ValueError("Hiányzó ADR mappa: docs/adr")
    files = list(ADR_DIR.glob("*.md"))
    if not files:
        raise ValueError("Legalább egy ADR markdown fájlnak lennie kell a docs/adr könyvtárban.")


def validate_sprint_one() -> None:
    if not DOCS_DIR.exists():
        raise ValueError("Hiányzik a docs mappa")
